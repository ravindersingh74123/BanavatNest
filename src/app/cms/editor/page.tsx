...
import {
  ...
  LogOut, Shield, ChevronRight, History, X
} from 'lucide-react';
...
type SectionKey = 'profile' | 'job' | 'research' | 'education' | 'contact';
...
export default function EditorPage() {
  ...
  useEffect(() => {
    fetch('/api/cms/auth/me')
      .then((r) => r.json())
      .then((u) => {
        if (!u.id) { router.push('/cms/login'); return; }
        setCurrentUser(u);
      })
      ...
      <nav className="sticky top-0 z-40 bg-white/90 dark:bg-[#09090b]/90 backdrop-blur-xl border-b border-zinc-100 dark:border-zinc-800 h-16 flex items-center px-4 sm:px-6 gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-2 pl-2 border-l border-zinc-100 dark:border-zinc-800">
            ...
            <button onClick={handleLogout} className="text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-colors">
        ...
        <aside className="hidden md:flex flex-col w-56 shrink-0 border-r border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 sticky top-16 h-[calc(100vh-4rem)] overflow-y-auto">
          ...
          <div className="mt-auto p-4 border-t border-zinc-100 dark:border-zinc-800">
            ...
            {portfolioStatus === 'published' && (
              <a
                href={`/en/directors/${/* username from portfolio.id */ ''}`}
                target="_blank"
                rel="noopener"
                className="flex items-center gap-2 text-xs font-bold text-zinc-500 hover:text-[#5BBD4A] transition-colors"
              >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeSection === 'job' && (
                <JobSection
                  job={portfolio.job}
                  onChange={(job) => updateSection('job', job)}
                />
              )}
              {activeSection === 'research' && (
                <ResearchSection
                  research={portfolio.research}
                  onChange={(research) => updateSection('research', research)}
                />
              )}
